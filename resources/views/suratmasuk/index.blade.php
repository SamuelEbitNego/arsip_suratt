...
<div class="container">
     ...
     <!-- Form Pencarian -->
     <form action="{{ route('suratmasuk.store') }}" method="GET">
          <div class="row mb-4">
               <div class="col-md-2">
               ...
               <div class="col-md-2">
                    <select name="disposisi" class="form-control">
                         <option value="">Cari Disposisi</option>
     ...
     <div class="d-flex flex-wrap gap-2 mb-3">
          <a href="{{ route('suratmasuk.create') }}" class="btn btn-success">
               <i class="fas fa-plus"></i> Tambah Surat
          </a>

          <a href="{{ route('suratmasuk.export_surat_masuk_pdf', array_merge(request()->all(), ['action' => 'download'])) }}" class="btn btn-danger">
               <i class="fas fa-file-pdf"></i> Export to PDF
          </a>

          <a href="{{ route('suratmasuk.export_masuk_excel', array_merge(request()->all(), ['action' => 'download'])) }}" class="btn btn-success">
               <i class="fas fa-file-excel"></i> Export to Excel
          </a>

          <a href="{{ route('suratmasuk.export_surat_masuk_pdf', array_merge(request()->all(), ['action' => 'preview'])) }}" class="btn btn-dark">
               <i class="fa-solid fa-eye"></i> Preview PDF
          </a>
     </div>

     <div class="container">
          <form action="{{ route('suratmasuk.import') }}" method="POST" enctype="multipart/form-data">
               @csrf
          ...
          <table class="table table-bordered table-hover mt-3">
               <thead class="thead">
                    <tr>
                         ...
                         <th rowspan="2" class="align-middle text-center sticky">Dari</th>
                         <th rowspan="2" class="align-middle text-center sticky">Nama Surat</th>
                         <th rowspan="2" class="align-middle text-center sticky">Keterangan</th>
                         ...
                         <th rowspan="2" class="align-middle text-center sticky">Tanggal Diterima</th>
                         ...
                         <th rowspan="2" class="align-middle text-center sticky">Disposisi</th>
               ...
               <tbody>
                    ...
                    <tr>
                         ...
                         <td class="text-center">{{ $s->dari }}</td>
                         <td>{{ $s->nama_surat }}</td>
                         ...
                         <td class="text-center">
                              <a href="{{ route('suratmasuk.edit', $s->id) }}" class="btn btn-warning btn-sm" title="Edit Surat">
                                   <i class="fas fa-edit"></i>
                              </a>
                              <form action="{{ route('suratmasuk.destroy', $s->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus surat ini?');">
                                   @csrf
                                   @method('DELETE')
